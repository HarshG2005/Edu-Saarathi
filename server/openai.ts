...
export function getGemini(): GoogleGenAI {
  if (!process.env.GEMINI_API_KEY) {
    throw new Error("GEMINI_API_KEY is not configured. Please add your Gemini API key in the Secrets tab.");
  }
...
Respond with JSON in this exact format:
{
  ...
  const response = await ai.models.generateContent({
    ...
    config: {
      systemInstruction: "You are an expert educator creating assessment questions. Generate high-quality multiple choice questions that test understanding at the appropriate difficulty level. Always respond with valid JSON only.",
...
Respond with JSON in this exact format:
{
  ...
  const response = await ai.models.generateContent({
    model: "gemini-2.5-flash",
    config: {
      systemInstruction: "You are an expert educator creating study materials. Generate effective flashcards that help students memorize and understand key concepts. Always respond with valid JSON only.",
      responseMimeType: "application/json",
    },
    contents: prompt,
  });
...
Respond with JSON in this exact format:
{
  ...
  const response = await ai.models.generateContent({
    model: "gemini-2.5-flash",
    config: {
      systemInstruction: "You are an expert at summarizing educational content. Create clear, well-organized summaries that capture the essential information. Always respond with valid JSON only.",
      responseMimeType: "application/json",
    },
    contents: prompt,
  });
...
Respond with JSON in this exact format:
{
  ...
  const response = await ai.models.generateContent({
    model: "gemini-2.5-flash",
    config: {
      systemInstruction: "You are an expert at organizing information visually. Create logical, well-structured mindmaps that help learners understand relationships between concepts. Always respond with valid JSON only.",
      responseMimeType: "application/json",
    },
    contents: prompt,
  });
...
Respond with JSON in this exact format:
{
  ...
  const response = await ai.models.generateContent({
    model: "gemini-2.5-flash",
    config: {
      systemInstruction: "You are an expert at extracting and organizing educational content. Create comprehensive, well-organized study notes that capture all essential information. Always respond with valid JSON only.",
      responseMimeType: "application/json",
    },
    contents: prompt,
  });
